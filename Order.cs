...
namespace reallyreallylast
{
    public partial class Order : Form
    {
        ...
        private void btnRemoveInOrder_Click(object sender, EventArgs e)
        {
            ...
            int total = Convert.ToInt32(dataCartsInOrder.Rows[selectedRow].Cells["UnitPrice"].Value);
            string sql = "DELETE FROM carts WHERE UnitPrice = '" + total + "'";
            MySqlCommand cmd = new MySqlCommand(sql, con);
            con.Open();
            int total2 = Convert.ToInt32(labelTotalAmount.Text);
            labelTotalAmount.Text = (total2 - total).ToString();
            DialogResult result = MessageBox.Show("ยืนยัน", "", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
        ...
        {
            DialogResult re = MessageBox.Show("ยืนยันการเพิ่ม", "ยืนยัน", MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
            if (re == DialogResult.OK)
            {
                string sql = "INSERT INTO carts(id,UnitPrice,name,Quanitity,total,admin)VALUES('" + id + "','" + txtPriceItemsInOrder.Text + "','" + txtNameInOrder.Text + "','" + txtQuanitityInOrder.Value + "','" + txtTotalInOrder.Text + "','"+admin+"')";
                MySqlCommand cmd = new MySqlCommand(sql, con);
                con.Open();
                cmd.ExecuteNonQuery();
                con.Close();
